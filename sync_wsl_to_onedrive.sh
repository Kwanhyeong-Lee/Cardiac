#!/bin/bash
# ============================================================
# WSL → OneDrive 시뮬레이션 데이터 동기화 스크립트
# ============================================================
# 용도: WSL 내부에 생성된 CFD 시뮬레이션 시간 디렉토리와
#       postProcessing 결과를 OneDrive로 복사
#
# 실행 방법: WSL Ubuntu 터미널에서
#   bash "/mnt/c/Users/alex0/OneDrive/PINN/260421 (심장) - main/sync_wsl_to_onedrive.sh"
#
# 주의: OneDrive 용량 확인 필요 (시뮬레이션 데이터는 수 GB 가능)
# ============================================================
set -e

# ---- 경로 설정 ----
WSL_CFD_DIR="$HOME/cardiac_cfd/lv_cfd_anatomical"
ONEDRIVE_BASE="/mnt/c/Users/alex0/OneDrive/PINN/260421 (심장) - main"
ONEDRIVE_CFD="${ONEDRIVE_BASE}/lv_cfd_anatomical"

echo "============================================================"
echo " WSL → OneDrive 시뮬레이션 데이터 동기화"
echo "============================================================"
echo ""
...
echo ""
echo "[3/4] 동기화 실행..."
mkdir -p "${ONEDRIVE_CFD}/postProcessing"
...
case $SYNC_MODE in
    a)
        ...
        rsync -av --progress \
            ...
            "$WSL_CFD_DIR/" "${ONEDRIVE_CFD}/"
        ;;
    b)
        echo "  결과만 동기화..."
        [ -d "$WSL_CFD_DIR/postProcessing" ] && cp -rv "$WSL_CFD_DIR/postProcessing/"* "${ONEDRIVE_CFD}/postProcessing/" 2>/dev/null
        cp -v "$WSL_CFD_DIR"/log.* "${ONEDRIVE_CFD}/" 2>/dev/null || true
        if [ -d "$WSL_CFD_DIR/constant/polyMesh" ]; then
            mkdir -p "${ONEDRIVE_CFD}/constant/polyMesh"
            cp -rv "$WSL_CFD_DIR/constant/polyMesh/"* "${ONEDRIVE_CFD}/constant/polyMesh/" 2>/dev/null
        fi
        ;;
    c)
        ...
        for TD in $LAST_3; do
            TNAME=$(basename "$TD")
            mkdir -p "${ONEDRIVE_CFD}/${TNAME}"
            cp -rv "$TD/"* "${ONEDRIVE_CFD}/${TNAME}/" 2>/dev/null
        done
        [ -d "$WSL_CFD_DIR/postProcessing" ] && cp -rv "$WSL_CFD_DIR/postProcessing/"* "${ONEDRIVE_CFD}/postProcessing/" 2>/dev/null
        cp -v "$WSL_CFD_DIR"/log.* "${ONEDRIVE_CFD}/" 2>/dev/null || true
        ;;
...
echo ""
echo "[4/4] 검증"
echo "  동기화된 파일: $(find "${ONEDRIVE_CFD}" -type f 2>/dev/null | wc -l)개"
echo "  동기화된 크기: $(du -sh "${ONEDRIVE_CFD}" 2>/dev/null | cut -f1)"
echo ""
echo "============================================================"
echo "  완료. 다음 단계:"
echo "  cd \"${ONEDRIVE_CFD}\" && python3 postprocess_analysis.py"
echo "============================================================"

# ============================================================
# [추가] OneDrive 쪽 파일 무결성 체크
# ============================================================
echo ""
echo "============================================================"
echo " OneDrive 파일 무결성 체크"
echo "============================================================"

# 이번 세션에서 생성된 핵심 파일 확인
echo ""
echo "[A] STL 파일 (triSurface/) 확인:"
TRISURFACE="${ONEDRIVE_CFD}/constant/triSurface"
...
SHM="${ONEDRIVE_CFD}/system/snappyHexMeshDict"
if [ -f "$SHM" ]; then
...
echo "   - log.pimpleFoam"
echo ""
echo " 위 스크립트의 모드 (b)를 실행하면 이 데이터가 OneDrive로 복사됩니다."
echo "============================================================"
