...
`_handover/` 폴더(원본 OneDrive 폴더 안, 저장소 밖)에 두 개의 아카이브를 만들었다:
- `cardiac-phpinn-workbench.bundle` — 이 저장소 전체(히스토리 포함). `git clone 파일.bundle 폴더명`.
- `handover_assets_*.zip` — 저장소에서 뺀 **비데이터** 산출물: 최종 STL(v4, 절개, 팬텀 코어/상자, CFD 패치), GLB 뷰어 모델, 원고 v2–v11 docx, `frameA_print.blend`. 새 PC의 저장소 옆에 풀면 된다(재생성도 가능).
- 데이터(§3)는 아카이브에 넣지 않았다. 외장 SSD로 직접 복사한다.
